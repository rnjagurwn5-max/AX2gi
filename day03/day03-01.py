...
pandas head/tail/shape/info/colunms 를 사용해서 데이터 셋의 기본 정보를 화면에 순서대로 보여주는 streamlit 앱이다.
...
#st.file_uploade로 파일을 직접 올릴 수 있다, 아무것도 올리지 않으면
# 아래의 기존 로직대로 같은 폴더 타이타닉 파일을 그대로 찾아서 읽는다. 
uploade_file = st.file_uploader("Titanic.csv 파일을 직접 업로드 할 수 있습니다(선택사항)", type="csv")

if uploade_file is not None :
    df = pd.read_csv(upload_file)
else  :
    try: 
        #이 스트립트와 같은 폴더에 있는 타이타닉 파일 읽어 온다.
...
if df is not None :
    st.subheader("1) heade(): 데이터의 뒷부분 5개 행 미리보기")
    ...
    #Age 35세 이상 필터링
        ...
        o = df[df["Age"] >= 35]
        st.write(f":**{len(o)}명**") 
        st.dataframe(o [["Name", "Sex", "Age"]].head())
